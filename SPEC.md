...
12. 🚧 Phase 3 — **HR & Hiring**. Pass A ✅: `coaching_sessions.hr_reviewed_at` / `hr_reviewed_by` / `hr_review_notes` · `/hr` dashboard with 3 stat tiles + coaching review queue + worst-10 panel (90d, category-filterable, raw count) · sidebar HR link + middleware gate. Pass B ✅: `duties_template_items.scope` column ('ops' | 'hr') + 10-item HR daily seed · `/hr/duties` page reusing the existing duties engine (flat list for daily, since HR has no preload/loadout buckets) · Duties button on `/hr` landing. Pass C–F to come: candidates kanban · dispatcher interview view in `/daily` · per-candidate QR'd interviewee form · onboarding tracking. Incidents/accidents (Phase 3 #3 — separate from `vehicle_issues`) deferred until after HR is complete.
