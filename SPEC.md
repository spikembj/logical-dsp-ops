...
### 19e. Candidate-facing forms (`/forms/<token>`) — Phase 3 Pass E
Per-candidate QR / link. The candidate fills out forms HR has built (currently seeded with two: **Interviewee form** and **Onboarding form**) without logging in — the token in the URL is the credential. Server resolves the token via the service-role client and never exposes the `candidate_id` to the browser.

**HR side.**
- New "Candidate forms" card on `/hr/candidates/[id]` listing every active form, each with one of three states (not generated / link generated / submitted), and buttons: Generate-QR → modal with the QR svg + copy-link · Show link / QR on already-generated · Regenerate (rotates the token, wipes `submitted_at`) · Delete · View answers (read-only).
- `/hr/candidates/forms` — list of all candidate forms. Click into one → `/hr/candidates/forms/[slug]` for the question editor (drag / rename / Y-N-or-Text / active / delete, same pattern as everywhere else).
- New "Forms" button on the candidates header alongside Statuses / Onboarding template / Interview questions.
- `/hr/candidates/[id]/forms/[slug]` — HR-side read-only view of one candidate's answers. Unanswered questions show "— no answer —" so missing data is visible.

**Public side.**
- `/forms/<token>` is publicly accessible (middleware exempts it). The page resolves the token, renders the form with candidate name + question list, and accepts repeated submits — the model is edit-in-place. First submission stamps `submitted_at`; later edits update answers without changing the timestamp.
- Invalid / deleted tokens render a friendly "this link is not valid" message instead of a stack trace.
- Submissions go through `submitPublicForm()` which uses the service-role client to bypass RLS, validates the token, drops forged question_ids, and replaces all answers (no diffing needed at the table size).

**Security notes:**
- 24-char base64url token (`generateFormToken()`), unique-constrained in the DB. Brute-forceability is negligible at our scale.
- Rotate-token wipes submitted_at + user-agent. Answers stay (linked to invitation_id, not the token).
- Social Security Number intentionally NOT seeded on the onboarding form — capturing it via a public token-only URL is a bigger PII story than we should default to. HR can add the question themselves with a conscious decision.
