...
Management-only — dispatchers cannot reach the route (middleware gate) and never see the sidebar link. Three header stat tiles (Awaiting HR review · Reviewed this week · Top offender 90d), a coaching review queue, and a worst-10 panel.
...
**Out of Pass A** (deferred to later HR passes): HR daily checklist (Pass B) · candidates kanban with the 8 status buckets (Pass C) · dispatcher interview view inside `/daily` (Pass D) · per-candidate QR'd interviewee form with dual-form review (Pass E) · onboarding tracking (Pass F).
...
12. 🚧 Phase 3 — **HR & Hiring**. Pass A ✅ (shipped): `coaching_sessions.hr_reviewed_at` / `hr_reviewed_by` / `hr_review_notes` columns · `/hr` dashboard with 3 stat tiles + coaching review queue + worst-10 panel (90d, category-filterable, raw count) · sidebar HR link + middleware gate (`/hr/*` management-only). Pass B–F to come: HR daily checklist · candidates kanban · dispatcher interview view in `/daily` · per-candidate QR'd interviewee form · onboarding tracking. Incidents/accidents (Phase 3 #3 — separate from `vehicle_issues`) deferred until after HR is complete.
