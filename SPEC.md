...
**Inline onboarding-template editor.** Collapsible panel on `/hr/candidates` (sits below Manage Statuses). Drag to reorder, rename inline, Active toggle, trash to delete (cascades to all candidates' completion stamps for that item). 10 seeded items: I-9, W-4, Drug test scheduled, Drug test passed, Background check submitted, Background check cleared, Direct deposit set up, Trainer assigned, Start date confirmed, Uniform issued.

**Status admin gets one more toggle** — `is_onboarding` (next to `declined-flag`) so HR can flip the onboarding checklist behavior onto any custom status without code changes.
...
12. 🚧 Phase 3 — **HR & Hiring**. Pass A ✅: `coaching_sessions.hr_reviewed_at` / `hr_reviewed_by` / `hr_review_notes` · `/hr` dashboard with 3 stat tiles + coaching review queue + worst-10 panel · sidebar HR link + middleware gate. Pass B ✅: `duties_template_items.scope` column + 10-item HR daily seed · `/hr/duties`. Pass C.A ✅: `candidate_statuses` + `candidates` + `candidate_onboarding_template_items` + `candidate_onboarding_completion` tables · `drivers.candidate_id` FK · `/hr/candidates` collapsible-by-status kanban with inline status admin + dedup. Pass C.B ✅: `candidate_statuses.is_onboarding` flag + `convert_candidate_to_driver()` RPC · `/hr/candidates/[id]` detail page · onboarding checklist UI with inline template editor · Convert-to-driver dialog · `/hr/candidates/archive` with All/Hired/Declined/Other tabs and search. Then Pass D (dispatcher interview view in `/daily`) and Pass E (per-candidate QR'd interviewee form). Incidents/accidents (Phase 3 #3 — separate from `vehicle_issues`) deferred until after HR is complete.
