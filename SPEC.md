...
**Safety event mix:** Two donut charts side by side (Impacting / Non-impacting), each showing the per-event-type breakdown of the **most recent Netradyne upload**. The window is data-based: `max(safety_events.event_date)` defines the period end; the period start is *inferred* as `end − 6 days` (matches the user's weekly upload cadence). Once Pass 8 lands and `file_imports.period_start` is populated, the period will be read directly instead of inferred. Each donut has a legend list (type + count) sorted desc. Empty state shows a "no safety data yet" panel that links to the Import page.
