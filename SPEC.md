...
**Safety event mix:** Two donut charts side by side (Impacting / Non-impacting), each showing the per-event-type breakdown for the **previous completed Amazon week** (Sun-Sat). Window is calendar-based — not data-based — so the donut reflects "what just happened" even if a Netradyne import lands a few days late. Each donut has a legend list (type + count) sorted desc. Empty state per donut when no events of that severity.
