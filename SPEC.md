...
**Set-password flow (`/auth/set-password`).** Newly-invited users and anyone clicking a password-recovery link land here after `/auth/callback` exchanges their token for a session. Form asks for new password + confirmation, calls `supabase.auth.updateUser({ password })`, then forwards to `/`. The callback routes both `type=invite` and `type=recovery` here regardless of whether `next` was set — belt-and-suspenders against any link generated before `redirectTo` was wired up. Without this page, invites would land on the dashboard logged in but with no password, locking users out on their next session.
