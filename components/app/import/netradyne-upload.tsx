...
import {
  importNetradyneCsv,
  type NetradyneImportSummary,
} from "@/app/actions/netradyne-import";
import { cn } from "@/lib/utils";

export function NetradyneUpload() {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [dragOver, setDragOver] = useState(false);
  const [selected, setSelected] = useState<File | null>(null);
  const [summary, setSummary] = useState<NetradyneImportSummary | null>(null);
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const res = await importNetradyneCsv(fd);
      setSummary(res);
      if (res.ok) {
        toast.success(
          `Imported ${res.events_written ?? 0} event row${(res.events_written ?? 0) === 1 ? "" : "s"}.`,
        );
        router.refresh();
      } else {
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="mt-3 text-sm">
          Drop the Netradyne event CSV here, or{" "}
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          The Drivers Report CSV from the Netradyne portal, e.g.{" "}
          <code className="font-mono text-[11px]">
            Monthly Drivers-Report(Apr-26-2026).csv
          </code>
        </p>
      </div>
      ...
      {summary && summary.ok && summary.parsed && (
        <div className="rounded-xl border bg-card p-4 space-y-3">
          ...
          <dl className="grid grid-cols-2 gap-y-1.5 gap-x-4 text-sm">
            ...
            <dd>{summary.parsed.fleet_name ?? "—"}</dd>
            <dt className="text-muted-foreground">Period</dt>
            <dd>
              {summary.parsed.period_start} → {summary.parsed.period_end}
            </dd>
            ...
            {!!summary.events_replaced && (
              <>
                <dt className="text-muted-foreground">Prior rows replaced</dt>
                <dd>{summary.events_replaced}</dd>
              </>
            )}
