...
import {
  importPodDetailsPdf,
  type PodDetailsImportSummary,
} from "@/app/actions/pod-details-import";
import { cn } from "@/lib/utils";

export function PodDetailsUpload() {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [dragOver, setDragOver] = useState(false);
  const [selected, setSelected] = useState<File | null>(null);
  const [summary, setSummary] = useState<PodDetailsImportSummary | null>(null);
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const res = await importPodDetailsPdf(fd);
      setSummary(res);
      if (res.ok) {
        toast.success(
          `Imported ${res.rows_written ?? 0} POD row${
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="mt-3 text-sm">
          Drop the POD Details PDF here, or{" "}
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Photo on Delivery acceptance breakdown per driver, e.g.{" "}
          <code className="font-mono text-[11px]">
            US-LGCL-DUT7-Week16-2026NA-DA-POD-Details.pdf
          </code>
        </p>
      </div>
