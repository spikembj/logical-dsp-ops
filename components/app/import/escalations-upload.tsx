...
import {
  importEscalationsCsv,
  type EscalationsImportSummary,
} from "@/app/actions/escalations-import";
import { cn } from "@/lib/utils";

export function EscalationsUpload() {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [dragOver, setDragOver] = useState(false);
  const [selected, setSelected] = useState<File | null>(null);
  const [summary, setSummary] = useState<EscalationsImportSummary | null>(null);
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const res = await importEscalationsCsv(fd);
      setSummary(res);
      if (res.ok) {
        toast.success(
          `Imported ${res.escalations_written ?? 0} escalation${
            (res.escalations_written ?? 0) === 1 ? "" : "s"
          }.`,
        );
        router.refresh();
      } else {
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="mt-3 text-sm">
          Drop the Escalations / Infractions CSV here, or{" "}
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          One row per individual infraction reported by Amazon, e.g.{" "}
          <code className="font-mono text-[11px]">
            US_LGCL_DUT7_WEEK16_2026_Escalations_(Infractions)_Report.csv
          </code>
        </p>
      </div>
      ...
      {summary && summary.ok && summary.parsed && (
        <div className="rounded-xl border bg-card p-4 space-y-3">
          ...
          <dl className="grid grid-cols-2 gap-y-1.5 gap-x-4 text-sm">
            ...
            <dt className="text-muted-foreground">Escalation rows</dt>
