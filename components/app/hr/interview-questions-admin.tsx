...
/**
 * Question template editor for the dispatcher interview form. Same
 * drag/rename/active/delete pattern as the onboarding template. Adds
 * a Y/N vs Text picker per row (the response_type) since the question
 * shape determines how the dispatcher form renders it.
 */
export function InterviewQuestionsAdmin({
  ...
  function commitOrder(next: InterviewQuestion[]) {
    setOrder(next);
    startTransition(async () => {
      const res = await reorderInterviewQuestions({
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        Questions appear on every candidate&rsquo;s dispatcher interview
        form. Yes/No questions render as a 3-state chip (Yes / No / —),
        text questions as a textarea. Toggle <strong>active</strong> off
        to retire a question without losing past answers.
      </p>
    </div>
  );
}
...
function QuestionRow({
  ...
  function patch(
    fields: Partial<Pick<InterviewQuestion, "prompt" | "response_type" | "active">>,
  ) {
    startTransition(async () => {
      const res = await upsertInterviewQuestion({
        id: question.id,
        prompt: fields.prompt ?? question.prompt,
        response_type:
          (fields.response_type ?? question.response_type) as InterviewResponseType,
  ...
  function handleDelete() {
    if (
      ...
      return;
    startTransition(async () => {
      const res = await deleteInterviewQuestion({ question_id: question.id });
      if (!res.ok) {
        toast.error(res.error);
        return;
      }
  ...
  return (
    ...
    >
      ...
      <select
        ...
        onChange={(e) =>
          patch({ response_type: e.currentTarget.value as InterviewResponseType })
        }
...
function AddQuestionRow({
  ...
  const [type, setType] = useState<InterviewResponseType>("yn");
  const [pending, startTransition] = useTransition();
  ...
  return (
    <div className="flex items-center gap-2">
      <select
        value={type}
        onChange={(e) => setType(e.currentTarget.value as InterviewResponseType)}
        disabled={pending}
